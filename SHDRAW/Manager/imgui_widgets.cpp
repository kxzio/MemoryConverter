...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImColor EstimatedColor = hovered ? ImColor(61, 135, 252) : ImColor(35, 35, 35, 150);
    ImColor pAnimatedColor;
    pAnimatedColor.Value.x = Animate(label, "AnimateColorR", true, EstimatedColor.Value.x - pAnimatedColor.Value.x, 7, INTERP);
    pAnimatedColor.Value.y = Animate(label, "AnimateColorG", true, EstimatedColor.Value.y - pAnimatedColor.Value.y, 7, INTERP);
    pAnimatedColor.Value.z = Animate(label, "AnimateColorB", true, EstimatedColor.Value.z - pAnimatedColor.Value.z, 7, INTERP);
    pAnimatedColor.Value.w = Animate(label, "AnimateColorA", true, EstimatedColor.Value.w - pAnimatedColor.Value.w, 7, INTERP);

    ImColor EstimatedColorForText = hovered ? ImColor(0, 0, 0, 255) : ImColor(255, 255, 255);
    ImColor pAnimatedColorForText;
    pAnimatedColorForText.Value.x = Animate(label, "AnimateColorRForText", true, EstimatedColorForText.Value.x - pAnimatedColorForText.Value.x, 7, INTERP);
    pAnimatedColorForText.Value.y = Animate(label, "AnimateColorGForText", true, EstimatedColorForText.Value.y - pAnimatedColorForText.Value.y, 7, INTERP);
    pAnimatedColorForText.Value.z = Animate(label, "AnimateColorBForText", true, EstimatedColorForText.Value.z - pAnimatedColorForText.Value.z, 7, INTERP);
    pAnimatedColorForText.Value.w = Animate(label, "AnimateColorAForText", true, EstimatedColorForText.Value.w - pAnimatedColorForText.Value.w, 7, INTERP);

    if (true) {
        int HoveredRectAlpha = Animate(label, "ButtonHovered", hovered, 255, 18, STATIC);
        window->DrawList->AddRectFilled(bb.Min, bb.Max, pAnimatedColor, 5);
        window->DrawList->AddRect(bb.Min, bb.Max, pAnimatedColor, 5);
        window->DrawList->AddRect(bb.Min, bb.Max, ImColor(61, 135, 252, HoveredRectAlpha), 5);
    }
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (true) {
        int HoveredRectAlpha = Animate(label, "ButtonHovered", hovered, 255, 18, STATIC);
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(35, 35, 35, 150), 5);
        window->DrawList->AddRect(check_bb.Min, check_bb.Max, pAnimatedColor, 5, 15, 0.3);
    }
    int alpha = Animate(label, "default_active", *v, 255, 15, STATIC);
    const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 3.0f));
    RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), ImColor(61, 135, 252, alpha), square_sz - pad * 2.0f);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    float move_delta = Animate(label, "move_delta", selected, 20, 10, DYNAMIC);

    int alpha_delta = Animate(label, "alpha_delta", selected, 255, 15, STATIC);
    window->DrawList->AddRectFilled(bb.Min, bb.Min + ImVec2(3, bb.GetSize().y), ImColor(61, 135, 252, alpha_delta));
    window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max,
    ImColor(61, 135, 252, int((alpha_delta / 255.f) * 30.f)), ImColor(61, 135, 252, 0), ImColor(61, 135, 252, 0), ImColor(61, 135, 252, int((alpha_delta / 255.f) * 30.f)));
    
    const float pad = ImMax(1.0f, IM_FLOOR(20 / 3.0f));
    RenderCheckMark(window->DrawList, bb.Min + ImVec2(pad + 2, pad - 2), ImColor(61, 135, 252, alpha_delta), 20 - pad * 2.0f);
    ...
    PushStyleColor(ImGuiCol_Text, ImVec4(ImColor(255, 255, 255, 50 + alpha_delta)));
    RenderTextClipped(text_min + ImVec2(2, 0) + ImVec2(move_delta, 0), text_max + ImVec2(move_delta, 0) + ImVec2(2, 0), label, NULL, &label_size, style.SelectableTextAlign, &bb);
    PopStyleColor();
